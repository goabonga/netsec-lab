...
# `assets/netsec-lab.svg`. Re-run after editing the master SVG and commit
# the outputs - the docs job builds the site from the checked-in files
# and never runs this script.
#
# Outputs:
#   docs/netsec-lab.svg   site logo   (zensical `[project.theme].logo`)
...
SRC="$ROOT/assets/netsec-lab.svg"
...
cp "$SRC" "$ROOT/docs/netsec-lab.svg"
...
uv run --only-group favicon python "$ROOT/scripts/generate_favicon.py" \
  -i "$ROOT/docs/netsec-lab.svg" \
  -o "$ROOT/docs/favicon.ico"

echo "regen-icons: docs/netsec-lab.svg + docs/favicon.ico from $SRC"
